...
public class ClasspathResourceUpDownloadController {
    ...
    public ResponseEntity<Resource> getFile(@PathVariable String name) throws IOException {
    	...
    	if(!imageFile.exists()) {
    		throw new ResponseStatusException(HttpStatus.NOT_FOUND,"찾지 못함 ");
    	}
    	HttpHeaders headers = new HttpHeaders();
    	headers.setContentLength(imageFile.contentLength());
    	headers.setContentDisposition(ContentDisposition.attachment().filename(name, Charset.forName("UTF-8")).build());
    ...
    public String uploadFiles(@RequestPart MultipartFile[] uploadFile) throws IOException {
    	 ...
    	 File imageFileFolder = imageFolder.getFile();
    	 Arrays.stream(uploadFile).filter(f->!f.isEmpty()).forEach(Failable.asConsumer(f->{
    		 File newFile = new File(imageFileFolder, f.getOriginalFilename());
